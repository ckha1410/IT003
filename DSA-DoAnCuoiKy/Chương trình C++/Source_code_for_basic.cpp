/**
    XÂY DỰNG CHƯƠNG TRÌNH CƠ BẢN CÓ DỮ LIỆU ĐƯỢC CHO SẴN
...
int main()
{
    /**
     ...
     XÂY DỰNG CHƯƠNG TRÌNH CHẠY SẴN:
    ...
    **/
    ...
    //store the strings that we want to list by a prefix in the trie
    vector<char*>listQuery = {"an" ,"br", "se", "h"};
    int len5 = listQuery.size();  //number of delete operations in the trie
    /// listing
    cout << "Listing:";
    for (int i = 0; i < len5; i++)
    {
        cout << "\nWords are listed by the prefix \"" << listQuery[i] << "\": ";
        list_word_byPrefix(root, listQuery[i]);
    }
